...
public class CourseSchedule2_dfs_without_reversion {
    /**
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        int seq = numCourses - 1;
        for(int i=0;i<numCourses;i++){
            if(color[i]=='W' && seq>=0) {
                seq = dfs(i, color, result, adjList, seq);
            }
        }
    ...
    public int dfs(int vertex, char[] color, int[] result,Map<Integer, List<Integer>> adjList, int seq){
        color[vertex] = 'G';
        for(int neighbour : adjList.getOrDefault(vertex, Collections.emptyList())){
            if(color[neighbour]=='W' && seq>=0){
                seq = dfs(neighbour, color, result, adjList, seq);
        ...
        return seq; //thus it is last one(0) , then return -1;
    }
}
