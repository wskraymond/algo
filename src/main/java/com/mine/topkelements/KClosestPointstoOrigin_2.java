...
public class KClosestPointstoOrigin_2 {
    ...
    public int[][] kClosest(int[][] points, int k) {
        ...
        Queue<int[]> maxHeap = new PriorityQueue<>(Comparator.comparingInt(p-> -sqDistFromOrigin(p)));
        for(int i=0;i<points.length;i++){ //O(n)
            maxHeap.add(points[i]); //O(log(k))
        ...
        return maxHeap.stream().toArray(int[][]::new); //O(k*log(k))
